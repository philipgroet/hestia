...
def privileged(update, context, command, check_only=True):
    admins = query_db("SELECT * FROM subscribers WHERE user_level = 9")
...
#    await context.bot.send_message(chat_id=secrets.OWN_CHAT_ID, text=log_msg)
    ...
    if reenable:
        query_db("UPDATE subscribers SET telegram_enabled = true WHERE telegram_id = %s", params=[str(update.effective_chat.id)])
    else:
        query_db("INSERT INTO subscribers (user_level, subscription_expiry, filter_min_price, filter_max_price, filter_cities, telegram_enabled, telegram_id) VALUES (DEFAULT, '2099-01-01T00:00:00', DEFAULT, DEFAULT, DEFAULT, true, %s)", params=[str(update.effective_chat.id)])
...
async def make_admin(update, context):
    query_db("UPDATE subscribers SET user_level = 9 WHERE telegram_id = %s", params=[str(update.effective_chat.id)])
    await context.bot.send_message(update.effective_chat.id, "You are now admin!")

async def start(update, context):
    checksub = query_db("SELECT * FROM subscribers WHERE telegram_id = %s", params=[str(update.effective_chat.id)], fetchOne=True)
...
async def stop(update, context):
    ...
    if checksub is not None:
        if checksub["telegram_enabled"]:
            ...
            query_db("UPDATE subscribers SET telegram_enabled = false WHERE telegram_id = %s", params=[str(update.effective_chat.id)])
...
#            await context.bot.send_message(chat_id=secrets.OWN_CHAT_ID, text=log_msg)
    ...
    donation_link = query_db("SELECT donation_link FROM meta", fetchOne=True)["donation_link"]
...
async def websites(update, context):
    # targets = query_db("SELECT agency, website FROM targets WHERE enabled = true")
...
async def halt(update, context):
    ...
    query_db("UPDATE meta SET scraper_halted = true WHERE id = %s", params=[hestia.SETTINGS_ID])
...
async def resume(update, context):
    ...
    settings = query_db("SELECT scraper_halted FROM meta WHERE id = %s", params=[hestia.SETTINGS_ID], fetchOne=True)
    
    if settings["scraper_halted"]:
        query_db("UPDATE meta SET scraper_halted = false WHERE id = %s", params=[hestia.SETTINGS_ID])
...
async def enable_dev(update, context):
    ...
    query_db("UPDATE meta SET devmode_enabled = true WHERE id = %s", params=[hestia.SETTINGS_ID])
...
async def disable_dev(update, context):
    ...
    query_db("UPDATE meta SET devmode_enabled = false WHERE id = %s", params=[hestia.SETTINGS_ID])
...
async def status(update, context):
    ...
    settings = query_db("SELECT * FROM meta WHERE id = %s", params=[hestia.SETTINGS_ID], fetchOne=True)
    ...
    sub_count = query_db("SELECT COUNT(*) FROM subscribers WHERE telegram_enabled = true", fetchOne=True)
    message += "\n"
    message += f"Active subscriber count: {sub_count['count']}\n"
    
    donation_link = query_db("SELECT donation_link, donation_link_updated FROM meta", fetchOne=True)
    message += "\n"
    message += f"Current donation link: {donation_link['donation_link']}\n"
    message += f"Last updated: {donation_link['donation_link_updated']}\n"

    # targets = query_db("SELECT * FROM targets")
    message += "\n"
    message += "Targets (id): listings in past 7 days\n"
        
    for target in targets:
        agency = target.agency
        count = query_db("SELECT COUNT(*) FROM homes WHERE agency = %s AND date_added > now() - '1 week'::interval", params=[agency], fetchOne=True)
...
async def set_donation_link(update, context):
    ...
    query_db("UPDATE meta SET donation_link = %s, donation_link_updated = %s WHERE id = %s", params=[link, datetime.now().isoformat(), hestia.SETTINGS_ID])
...
async def filter(update, context):
    ...
    # '/filter' only
    if len(cmd) == 1:
        sub = query_db("SELECT * FROM subscribers WHERE telegram_id = %s", params=[str(update.effective_chat.id)], fetchOne=True)
        ...
        message += f"Max distance to center (km): {sub['filter_distance_to_center']}\n\n"
        ...
        message += "`/filter distance 3.5` where the distance is in km\n\n"
    ...
    # Set minprice filter
        ...
        query_db("UPDATE subscribers SET filter_min_price = %s WHERE telegram_id = %s", params=(minprice, str(update.effective_chat.id)))
    ...
    # Set maxprice filter
        ...
        query_db("UPDATE subscribers SET filter_max_price = %s WHERE telegram_id = %s", params=(maxprice, str(update.effective_chat.id)))
        
        message = f"Maximum price filter set to {maxprice}!"

    # Set distance filter
    elif len(cmd) == 3 and cmd[1] in ["distance"]:
        try:
            distance = float(cmd[2])
        except ValueError:
            message = f"Invalid value: {cmd[2]} is not a float."
            await context.bot.send_message(update.effective_chat.id, message)
            return
            
        query_db("UPDATE subscribers SET filter_distance_to_center = %s WHERE telegram_id = %s", params=(distance, str(update.effective_chat.id)))
        
        message = f"Maximum distance to center filter set to {distance}!"
            
    # View city possibilities
    elif len(cmd) == 2 and cmd[1] == "city":
        all_filter_cities = [c["city"] for c in query_db("SELECT DISTINCT city FROM homes")]
    ...
    # Modify city filter
        ...
        sub_filter_cities = query_db("SELECT filter_cities FROM subscribers WHERE telegram_id = %s", params=[str(update.effective_chat.id)], fetchOne=True)["filter_cities"]
        ...
        if cmd[2] == "add":
            ...
            query_db("UPDATE subscribers SET filter_cities = %s WHERE telegram_id = %s", params=(sub_filter_cities, str(update.effective_chat.id)))
